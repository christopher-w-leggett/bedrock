...
/**
 * Responsible for exposing cache stats and cache variables in
 * <code>CacheService</code> instances.
 */
public abstract class AbstractCacheService implements CacheService {

	@Override
	public final boolean clearAllCaches() {
	...
	@Override
	public final boolean clearSpecificCache(final String cacheVariableName) {
		...
		for (final Field field : collectFields()) {
			if (isNamedCache(field, cacheVariableName)) {
				try {
					...
					getLogger().error(
						"An error has occurred while attempting retrieve cache size for " + field.getName()
							+ " in the class " + this.getClass().getName() + ".", e);
				}
			}
		}
		...
		for (final Field field : collectFields()) {
			if (isNamedCache(field, cacheVariableName)) {
				try {
					...
					getLogger().error(
						"An error has occurred while attempting retrieve cache statistics for " + field.getName()
							+ " in the class " + this.getClass().getName() + ".", e);
				}
			}
		}
